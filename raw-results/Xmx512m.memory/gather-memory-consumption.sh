...
gnuplot -p << EOF | cat
	...
	set format y "%.0f";
	set output "memory-iterations-$iterations.pdf";
	plot for [IDX=0:1] "$plot" index IDX using 3:4 title columnheader(1);
EOF
done
...
gnuplot -p << EOF | cat
	...
	set ylabel "Memory Usage (MB)";
	set format y "%.0f";
	set output "memory-time-$iterations.pdf";
	plot for [IDX=0:1] "$plot" index IDX using 1 title columnheader(1);
EOF
done
